...
export class DatabaseStorage implements IStorage {
  ...
  private async initializeSchema() {
    try {
      ...
      // 1. Ensure base tables exist and have necessary columns
      try {
        ...
        `);
        console.log("[Storage] leads columns verified/added.");
      } catch (err) {
        console.log("[Storage] leads column check (non-critical):", err);
      }
      ...
      try {
        ...
        console.log("[Storage] email_config update check...");
        try {
          await db.execute(sql`
            ALTER TABLE email_config ADD COLUMN IF NOT EXISTS user_id varchar REFERENCES users(id);
          `);
        } catch (colErr) {
          console.log("[Storage] email_config column update check failed:", colErr);
        }
      }
