...
export default function LeadDetailsModal({ lead, isOpen, onClose, onUpdate, readOnly = false }: LeadDetailsModalProps) {
  ...
  // Pass to Accounts mutation with inline validation
  const passToAccountsMutation = useMutation({
    mutationFn: async (data: UpdateLeadForm) => {
      const response = await apiRequest("POST", `/api/leads/${lead.id}/pass-to-accounts`, data);
      return response.json();
    },
    onSuccess: () => {
      toast({
        title: "Lead Passed to Accounts",
        description: "Lead has been successfully transferred to the Accounts team.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/leads"] });
      queryClient.invalidateQueries({ queryKey: ["/api/my/leads"] });
      queryClient.invalidateQueries({ queryKey: ["/api/my/completed"] });
    ...
    onError: (error: Error) => {
      ...
      toast({
        title: "Pass to Accounts Failed",
        description: error.message || "Please fill all required fields before passing to accounts.",
        variant: "destructive",
      });
    },
  });

  // Handle Pass to Accounts with inline validation
  const handlePassToAccounts = () => {
    ...
    passToAccountsMutation.mutate(data);
  };
  ...
  const onSubmit = (data: UpdateLeadForm) => {
    ...
    // Save Changes only saves the lead - no transfer to accounts
    // Use "Pass to Accounts" button to validate and transfer leads
          ...
          <TabsContent value="details" className="space-y-4">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                {/* Walk-in Details - Always visible for Pass to Accounts */}
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center">
                      <Calendar className="mr-2 h-4 w-4" />
                      Walk-in Details
                      <span className="ml-2 text-xs text-muted-foreground font-normal">(Required for Pass to Accounts)</span>
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} type="date" data-testid="input-walkin-date-main" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} type="time" data-testid="input-walkin-time-main" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} type="number" step="0.01" placeholder="e.g., 5000.00" data-testid="input-registration-amount-main" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input {...field} type="number" step="0.01" placeholder="e.g., 1000.00" data-testid="input-partial-amount" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input {...field} type="number" step="0.01" placeholder="e.g., 500.00" data-testid="input-concession" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input {...field} type="text" placeholder="e.g., TXN123456" data-testid="input-transaction-number" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
